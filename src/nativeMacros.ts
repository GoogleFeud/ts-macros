...
export default {
  ...
  $$inline: {
    call: ([func, params, doNotCall], transformer, callSite) => {
      ...
      const fn = normalizeFunctionNode(
        transformer.checker,
        ts.visitNode(func, transformer.boundVisitor)
      );
      if (!fn || !fn.body)
        throw MacroError(
          callSite,
    ...
    call: ([simplifyType, nonNullType], transformer, callSite) => {
      ...
      if (type.isTypeParameter()) {
        ...
        return ts.factory.createStringLiteral(
          transformer.checker.typeToString(getFinalType(param))
        );
      } else {
        const lastMacro = transformer.getLastMacro();
        if (lastMacro) {
          ...
          return ts.factory.createStringLiteral(
            transformer.checker.typeToString(getFinalType(type))
          );
      }
    },
  },
      ...
      if (ts.isExpressionStatement(parent)) {
        ...
        if ("body" in parent) {
          ...
          transformer.addComptimeSignature(
            ...
            fnBodyToString(transformer.checker, callableFn),
